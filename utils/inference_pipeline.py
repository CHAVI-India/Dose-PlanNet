...
_PROJECT_UTILS = str(Path(__file__).resolve().parent.parent.parent / "utils")
if _PROJECT_UTILS not in sys.path:
    sys.path.insert(0, _PROJECT_UTILS)
try:
    from create_dummy_plan import create_dummy_plan_dicom
except ImportError:
    create_dummy_plan_dicom = None
...
def run_inference(patient_id, images_dir, config, C, output_dir=".",
    ...
    try:
        ...
        if pred_arr.ndim == 4:   # (1, D, H, W) → (D, H, W)
            pred_arr = pred_arr[0]
        # MONAI Invertd returns spatial dims in (x,y,z) order (SimpleITK convention).
        # sitk.GetImageFromArray expects (z,y,x) numpy convention — transpose here
        # so both the Invertd path and the sitk-fallback path produce the same layout.
        pred_dose_native_np = np.clip(pred_arr.astype(np.float32), 0.0, None).transpose(2, 1, 0)
...
def run_pipeline(dicom_dir, config_path="config.yml", model_path=None,
    ...
    # Auto-create a dummy RTPLAN if none exists (required for TPS RTDOSE import)
    _has_rtplan = any(
        getattr(pydicom.dcmread(str(f), stop_before_pixels=True, force=True), "Modality", "") == "RTPLAN"
        for f in Path(dicom_dir).rglob("*.dcm")
    )
    if not _has_rtplan:
        if create_dummy_plan_dicom is not None:
            print("  No RTPLAN found — creating dummy plan...")
            try:
                create_dummy_plan_dicom(dicom_dir)
            except Exception as _e:
                print(f"  Dummy plan creation failed: {_e}")
        else:
            print("  No RTPLAN found and create_dummy_plan not available — RTDOSE will be unlinked.")
...
if __name__ == "__main__":
    ...
    group.add_argument("--dicom-dir", type=str, help="Single patient DICOM directory")
    group.add_argument("--batch-dir", type=str, help="Root folder; each sub-folder is one patient")
    parser.add_argument("--config", default="config/config.yml", type=str)
    parser.add_argument("--model", default="best_dose_model_clinical_june23.pth", type=str)
    ...
    if args.dicom_dir:
        ...
        patient_dirs = sorted([p for p in batch_root.iterdir() if p.is_dir()])
        print(f"\nBatch mode: {len(patient_dirs)} patient(s) found in {batch_root}\n")

        results = []
        for patient_dir in patient_dirs:
            # Resolve the actual DICOM directory (may be nested one level deeper)
            dicom_dir = str(patient_dir)
            subdirs = [d for d in patient_dir.iterdir() if d.is_dir()]
            if subdirs:
                dicom_dir = str(subdirs[0])
            ...
            try:
                rtdose_path = run_pipeline(
                    dicom_dir=dicom_dir, config_path=args.config,
                    model_path=args.model, dose_spacing_mm=args.dose_spacing,
                    keep_temp=args.keep_temp,
                )
                results.append((patient_dir.name, "OK", rtdose_path or ""))
            except Exception as exc:
                print(f"  [ERROR] {patient_dir.name}: {exc}")
                results.append((patient_dir.name, "FAILED", str(exc)))
