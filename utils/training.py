...
_CONFIG_FILE = os.path.join(_SCRIPT_DIR, "..", "config", "prostate_prime_experimental.yml")
...
# ===================================================================
_data_dir_raw = os.environ.get("DATA_DIR", config["dataset"]["data_dir"])
DATA_DIR = (
    _data_dir_raw
    if os.path.isabs(_data_dir_raw)
    else os.path.normpath(os.path.join(_SCRIPT_DIR, "..", _data_dir_raw))
)
...
def main(eval_only=False):
    ...
    if not eval_only:
        ...
        epoch_range = []
        print("\n[eval-only] Skipping training — running final evaluation on saved checkpoints.")
        model.eval()
    ...
    for epoch in epoch_range:
        ...
        if (epoch + 1) % VAL_EVERY_N_EPOCHS == 0:
            ...
            if is_physically_valid and val_loss_avg < best_val_loss:
                ...
                torch.save(model.state_dict(), "pe_best_dose_model_physics_june25.pth")
            ...
            if is_physically_valid and clinical_score < best_clinical_score:
                ...
                torch.save(model.state_dict(), "pe_best_dose_model_clinical_june25.pth")
                clinical_msg = (
            ...
            if val_loss_avg < best_diagnostic_mse:
                ...
                torch.save(model.state_dict(), "pe_best_dose_model_diagnostic_june25.pth")
    ...
    # ====================================================================
    # Clear GPU cache before full-volume evaluation to reduce fragmentation
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    for model_path, csv_name in [
        ("pe_best_dose_model_physics_june25.pth",       "validation_physics_summary_june25.csv"),
        ("pe_best_dose_model_clinical_june25.pth",      "validation_clinical_summary_june25.csv"),
        ("pe_best_dose_model_diagnostic_june25.pth",    "validation_diagnostic_summary_june25.csv"),
    ]:
        ...
        # SIB target mapping — built once per checkpoint, not per batch
        sib_eval_targets = {
            lvl["name"]: lvl["rx_gy"]
            for lvl in config["clinical_targets"]["targets"]
        }
        ...
        with torch.no_grad():
            for idx, batch in enumerate(val_loader):
                ...
                # Apply body mask then move to CPU immediately to avoid OOM
                ...
                outputs_gy = (outputs_gy * body_mask_eval).cpu()

                ptv_mask, bladder_mask, rectum_mask = extract_binary_masks(inputs)
                penile_mask_eval = (inputs[:, _PENILE_CH_IDX:_PENILE_CH_IDX+1, ...] > 0.5).cpu()

                discrete_ptv = inputs[:, _PTV_CH_IDX:_PTV_CH_IDX+1, ...].cpu()

                bladder_dose = outputs_gy[bladder_mask.cpu().bool()]
                rectum_dose  = outputs_gy[rectum_mask.cpu().bool()]
                penile_dose  = outputs_gy[penile_mask_eval.bool()]
                ...
                for name, id_val in sib_eval_targets.items():
                    mask = torch.isclose(discrete_ptv, torch.tensor(id_val, dtype=torch.float32, device=discrete_ptv.device))
                    sib_dose = outputs_gy[mask.bool()].cpu()
                ...
                print(
                    ...
                    f"{_PTV_PRIMARY_NAME} D95={row.get(f'{_PTV_PRIMARY_NAME}_D95 (Gy)', float('nan')):.2f} Gy  "
                    f"{_PTV_SECONDARY_NAME} D95={row.get(f'{_PTV_SECONDARY_NAME}_D95 (Gy)', float('nan')):.2f} Gy  "
                ...
                # Clean up GPU memory to prevent OOM during full-volume inference
                del inputs, outputs, body_mask_eval
                del ptv_mask, bladder_mask, rectum_mask
                del outputs_gy, penile_mask_eval, discrete_ptv
                del bladder_dose, rectum_dose, penile_dose
                gc.collect()
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train or evaluate dose prediction model")
    parser.add_argument("--eval-only", action="store_true",
                        help="Skip training and run final evaluation on saved checkpoints")
